use std::{sync::Arc, sync::atomic::{AtomicU32, Ordering}, time::Instant};
use colored::*;
use drillx::{
    equix::{self},
    Hash, Solution,
                        ...
                        loop {
                            ...
                            if let Ok(hx) = drillx::hash_with_memory(
                                ...
                                if difficulty.gt(&best_difficulty) {
                                    ...
                                    best_hash = hx;
                                    // Update the global best difficulty if this thread found a better one
                                    global_best_difficulty.store(best_difficulty, Ordering::SeqCst);
                                }
                            }
                            ...
                            if nonce % 1000 == 0 {
                                let global_best_difficulty =
                                    global_best_difficulty.load(Ordering::SeqCst);
